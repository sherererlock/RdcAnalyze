...
_ANY_ACTION = _DRAWCALL | _MESHDRAW | _DISPATCH | _CLEAR | _COPY | _RESOLVE | _GENMIPS


def _subtree_has_draws(action: Any) -> bool:
    if int(action.flags) & _ANY_ACTION:
        return True
...
def _window_stats(begin: Any, window: list[Any], sf: Any = None) -> dict[str, Any]:
    ...
    draws = dispatches = copies = clears = triangles = 0
    eids = [begin.eventId] + [w.eventId for w in window]
    min_eid, max_eid = min(eids), max(eids)

    def _walk(a: Any) -> None:
        nonlocal draws, dispatches, copies, clears, triangles, min_eid, max_eid
        flags = int(a.flags)
        ...
        if flags & (_DRAWCALL | _MESHDRAW):
            ...
            dispatches += 1
        elif flags & (_COPY | _RESOLVE | _GENMIPS):
            copies += 1
        elif flags & _CLEAR:
            clears += 1
        for c in a.children:
...
_DRAW_OR_DISPATCH_OR_CLEAR = _DRAWCALL | _MESHDRAW | _DISPATCH | _CLEAR | _COPY | _RESOLVE | _GENMIPS
...
def _build_synthetic_pass_list(actions: list[Any], sf: Any = None) -> list[dict[str, Any]]:
    ...
    def _flush() -> None:
        nonlocal cur_draws, cur_dispatches, cur_copies, cur_clears, cur_triangles, cur_begin_eid, cur_end_eid, cur_marker
        if cur_draws == 0 and cur_dispatches == 0 and cur_copies == 0 and cur_clears == 0:
            return
        if cur_marker:
            name = cur_marker
        elif cur_draws == 0 and cur_dispatches > 0 and cur_copies == 0 and cur_clears == 0:
            name = f"Compute Pass #{len(passes) + 1}"
        elif cur_draws == 0 and cur_dispatches == 0:
            name = f"Copy/Clear Pass #{len(passes) + 1}"
        else:
            name = _friendly_rt_name(cur_key, len(passes))
        passes.append(
            {
                "name": name,
    ...
    for a in leaf_actions:
        flags = int(a.flags)
        key = _rt_key(a)
        eid_gap = a.eventId - cur_end_eid if cur_end_eid else 0
        if key != cur_key or eid_gap > 10:
            _flush()
            cur_key = key
            cur_draws = 0
            cur_dispatches = 0
...
def _renumber_passes(passes: list[dict[str, Any]]) -> None:
    """Renumber passes per-type to match RenderDoc GUI convention."""
    counters = {"colour": 0, "compute": 0, "copyclear": 0}
    for p in passes:
        name = p.get("name", "")
        d = p.get("draws", 0)
