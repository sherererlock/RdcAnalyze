...
flowchart TD
    ...
    subgraph POST_MERGE ["后处理阶段"]
        ...
        S75["Step 7.5: TSV 导出<br/>export_tsv()<br/>(含 pipeline_stages/shader 分析)"]
...
flowchart TD
    ...
    subgraph ANALYSIS ["分析模块 (并行独立)"]
        ...
        A5["analyze_shaders()<br/>SPIR-V 复杂度/指令分布/<br/>寄存器压力/变体去重/热力图"]
    ...
    subgraph RENDER_SECTIONS ["HTML 报告章节"]
        ...
        H5["06 Shader Complexity — Shader 表格<br/>+ Instruction Mix + Variants + Heatmap"]
